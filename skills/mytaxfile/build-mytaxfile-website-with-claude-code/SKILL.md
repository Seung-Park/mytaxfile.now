...
- Colors (updated again 2026-08-01 — **cool/institutional palette adopted by Seung Park after a side-by-side preview**, superseding the warm terracotta palette from earlier the same day, which had itself superseded the original dark-navy palette): cool near-white background `#FAFBFC`, navy-slate ink `#1C2B3A` (headings/text), teal `#0F766E` (CTA/action), muted gold `#C58A2A` (limited accent, footer headings), deep navy `#0B2545` (footer background). All colors are tokenized in `website/css/styles.css` `:root` — a palette change is a single edit to that block; never hard-code a hue elsewhere.

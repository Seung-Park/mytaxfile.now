...
15. **Never commit, push, merge, create a PR, or deploy without Seung Park's express approval.** (**Amended 2026-08-07.** The CLAUDE.md standing authority of 2026-08-01 previously treated branch commits, pushes, and PRs within approved scope as Tier 1–2. Seung Park's pre-launch rules of 2026-08-07 narrowed that: for the duration of the pre-launch period, **commit, push, deploy, and publish each require separate express approval**, alongside merges and deployments. This rule now reads exactly as written in its first sentence, with no Tier 1–2 carve-out. See area 27.)
...
### 27. Pre-launch hosting, indexing, and publication controls

Approved by Seung Park 2026-08-07. In force for the whole pre-launch period — until he gives **final launch approval**. These rules govern hosting and visibility only; they change no page content, design, service scope, or pricing.

1. **Keep the Netlify site reachable for review.** The site stays available through its netlify.app URL so Seung and ChatGPT/Codex can review it. Verified review URL: `https://mytaxfilenow.netlify.app` (Netlify project `mytaxfilenow`, site ID `804f1646-c093-49f6-8a15-371d0370cc85`, branch URL `https://main--mytaxfilenow.netlify.app`, admin `https://app.netlify.com/projects/mytaxfilenow`). Do not add password protection, SSO gating, or any other access control that would break review access.
2. **The entire website stays `noindex` until final launch approval.** Every public page carries `<meta name="robots" content="noindex, nofollow" />`, and `website/_headers` sends `X-Robots-Tag: noindex, nofollow` for every path. Both mechanisms are required: the header also covers non-HTML assets, which cannot carry a meta tag, and it applies to every domain the project serves. **Do not add a `robots.txt` `Disallow: /` rule** — blocking the crawl prevents search engines from reading the `noindex` directive, which is the common way a site stays indexed by accident. Any new page must carry the meta tag from the moment it is created. Removing either mechanism is a launch decision requiring Seung Park's express approval.
3. **Do not deactivate, delete, unpublish, lock, disable, or rename the Netlify site**, and do not remove or change its netlify.app subdomain. This is explicit and absolute, not merely "with approval."
4. **Do not change the custom-domain or DNS settings.** Seung Park will temporarily disconnect the domain `mytaxfile.now` himself. Claude Code makes no domain, DNS, primary-domain, HTTPS, or certificate change during pre-launch — not even with approval-by-inference. When the domain is disconnected, register rows 146–147 become historical rather than current, and the netlify.app URL is the sole review URL.
5. **Do not invent contact information, legal company information, integrations, or operational claims.** This restates and does not weaken CLAUDE.md rule 15 ("never fabricate an operational fact"), area 24's standing rules, and the Proposed / unverified list. Unknown operational facts use a visibly marked `<span class="placeholder">`.
6. **Preserve all existing approved pages, sections, content, functions, and project-specific requirements.** Restates rules 3–4 and job-allocation item 5. Approved-content removals stay at zero.
7. **Do not commit, push, deploy, or publish without separate approval.** This narrows the 2026-08-01 standing authority — see rule 15 above. Commits and pushes are no longer Tier 1–2 during pre-launch.

**Known interaction.** A `noindex` change has no effect until it is deployed, and deploying requires rule 7 approval. Indexing protection and its deployment must therefore be approved together, as they were on 2026-08-07.

**Implemented 2026-08-07** on Seung Park's express approval of both recording and deployment. See the Change history entry and requirements-register rows 152–158.
...
- **2026-08-07** — **Pre-launch hosting, indexing, and publication controls approved by Seung Park** and added as area 27: keep the Netlify site reachable at its netlify.app URL for review; keep the entire website `noindex` until final launch approval; never deactivate or delete the Netlify site; make no custom-domain or DNS change (Seung disconnects `mytaxfile.now` himself); invent no contact, legal-entity, integration, or operational facts; preserve all approved pages, sections, content, functions, and requirements; and obtain separate approval to commit, push, deploy, or publish. Rule 15 amended in place — the Tier 1–2 carve-out for branch commits, pushes, and PRs is withdrawn for the pre-launch period, resolving the standing contradiction between rule 15 and area 22. Rules 5 and 6 were already fully covered by CLAUDE.md rule 15, area 24, rules 3–4, and job-allocation item 5; they are cross-referenced in area 27 rather than duplicated. Indexing protection implemented and deployed the same day on express approval: `<meta name="robots" content="noindex, nofollow" />` added to all 14 public pages and a new `website/_headers` sending `X-Robots-Tag: noindex, nofollow` for all paths. No page content, design, palette, route, Section ID, or copy was changed. Register rows 152–158.

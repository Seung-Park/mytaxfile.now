...
| 125 | **Open defect logged — primary CTA fails WCAG AA** — **CLOSED 2026-08-01 by the cool-palette adoption (row 126): CTA fill is now white on `#0f766e` = 5.47:1** (found during the 2026-08-01 palette review; NOT fixed by this pass, which changes no rendered colour) | `.cta-button` white label on `--color-accent` `#d97742` = **3.15:1**; hover `#c2632f` = **4.10:1**; both below the 4.5:1 normal-text threshold (button text is 17px/600 — not WCAG large text). Only `:active` `#a8531f` (5.35:1) passes. Row 85's "fixed site-wide" covered text-on-background usages only; white-on-accent **fills** were not in its scope. Fix requires darkening the brand CTA colour (candidate `#b8551f` ≈ 4.8:1) — **Tier 3 (brand colour change), awaiting Seung Park's decision.** Related: hero overlay minimum and semantic-token layer proposed as Steps 3–4, also awaiting go |

## Cool/institutional palette adopted — 2026-08-01 (express approval by Seung Park: "Adopt cool")

| # | Subject | Detail |
|---|---|---|
| 126 | **Cool/institutional palette adopted — supersedes rows 21/23 (warm terracotta / white-first warm)** | Approved after a side-by-side preview (Home, Resources, Privacy Notice desktop + Home mobile; current vs. cool). Values, anchored to the row-19 navy/teal/gold family: body `#fafbfc`, ink `#1c2b3a`, ink-soft `#48596a`, action `#0f766e`, hover `#0c5f59`, pressed `#094c47`, link text `#0e6b64`, tint `#d9eeeb`, gold `#c58a2a`, footer `#0b2545`, border `#e2e8ee`. Amber warning family, calculator scope badges, whites, and all layout/typography/spacing unchanged. The **white-first dominance rule of row 23 carries over unchanged** — only the hues changed. `--color-surface-warm` renamed `--color-surface-soft` (value now cool; both usages in the same file) |
| 127 | Verification of the applied palette | (a) All 18 text/UI contrast pairs computed before application — every pair ≥ WCAG AA (lowest: footer gold heading 5.16:1; CTA fill 5.47:1; focus ring 7.25:1 vs. a 3:1 threshold). (b) Repo pages screenshotted after application: the four previewed page/viewport combinations are **byte-identical (SHA-256) to the preview Seung approved**; remaining pages captured and spot-checked. (c) Grep confirms zero warm-era hexes remain in `styles.css` |
| 128 | Row-80 action refreshed | Website Plan v2 §8 still says "white background and deep navy" — it was stale against the warm palette and is now *closer* to true but still not the approved wording. **Action for Seung remains: update §8** to the adopted cool palette values (row 126). No Drive write performed this session — the Drive connector exposes no document-edit capability, and creating a new Doc would duplicate a canonical source |

## Approved master visual system applied site-wide — 2026-08-02 (express approval by Seung Park: "이 repository의 웹디자인은 이 style로 consistent하게 만들어줘")

| # | Subject | Detail |
|---|---|---|
| 129 | **Permanent job allocation approved** | Seung Park = product owner and final approval authority. **ChatGPT/Codex = visual design authority** (brand, colors, typography, composition, imagery, mockups, design contracts, visual acceptance). **Claude Code = principal technical implementation authority** (inspect first, implement approved design faithfully, accessible/responsive/maintainable code, verification deliverables, stop before merge/deploy). Recorded in `CLAUDE.md` rules 17–18 and the canonical Skill |
| 130 | **Approved master visual system** — the ChatGPT homepage mockup is the master visual reference; supersedes rows 126–127 (cool/institutional navy) as the current palette | Warm white `#FCFCFA`, surfaces `#FFFFFF`, slate headings `#294455`, slate body `#526A78`, teal actions `#168782` (hover/text `#0F706C`), pale blue-gray bands `#EFF6F7`, medium blue-gray footer `#4B7890` with full-white text. Mix target ~70–75% white, 15–20% pale blue-gray, 5–10% slate/teal. Deep navy and the orange-and-brown identity must not return. AA note: white on `#168782` is 4.35:1, so button/step text renders ≥18.7px semibold (WCAG large-text 3:1); text links use `#0F706C` (5.91:1) |
| 131 | **Style applied consistently to all 7 public pages** | v2 header (logo \| nav \| Sign In + short CTA + mobile toggle) and mockup footer (brand \| divider \| Services / Service Areas / Resources / Company & Legal \| bottom bar) replaced the global frame on index, Individual Tax, Business Tax, Resources, Privacy Notice, Terms of Use, Accessibility. `js/nav.js` wired on every page |
| 132 | **Index rebuilt keypoint-first per Seung's direction** ("많은말이 필요없고 organize되고 정리된 모습") | Order now: hero → trust strip → service cards (HOME-C-002) → 3-step "A simpler way to file." (HOME-C-003) → compressed keypoints (HOME-C-001, lede paragraph condensed to four ✓ points, all approved facts retained) → security (HOME-C-004, approved copy verbatim) → FAQ (HOME-C-005). Steps use the approved mockup 3-step wording, replacing the earlier 4-step copy on the homepage only. Hero eyebrow line removed per the mockup |
...
## Category page main images added — 2026-08-02 (express approval by Seung Park: "add some images with keywords on each of the category pages")

| # | Subject | Detail |
|---|---|---|
| 149 | **Compact category hero on all six category pages** | Individual Tax, Business Tax, Find My Service, Resources, About, Contact each open with a `.page-hero` band directly below the nav: eyebrow + short slogan + teal keyword line on the left, a curved main image bleeding to the right edge. Shorter than the homepage hero (min-height 280px vs. 480px). Reuses the approved palette; the `<img>` layer stays independently replaceable. Slogans/keywords: Individual "Your personal return, done right." / 1040·C·E; Business "Your business, filed properly." / 1120-S·K-1s·1065(conditional); Find My Service "Find the right place to start." / few questions·no jargon·clear next step; Resources "Numbers you can check yourself." / calculators·estimators·federal&state; About "Clarity before commitment." / plain language·honest scope·careful guidance; Contact "Tell us where you are stuck." / short overview·no sensitive docs·clear reply |
| 150 | **Six generated images added** | `website/images/category-*.webp`, 520×293 (about 500×281), 4.3–7.3 KB each, generated to match the approved bright blue-gray/white/teal lifestyle style. Transferred from the generation sandbox via checksummed base64 and each verified byte-identical (sha256). No people-identity or brand claims |
| 151 | Verification | All 6 pages: image loads (naturalWidth>0), no horizontal overflow at 390/768/1440, no console/network errors. Desktop and mobile screenshots reviewed |
